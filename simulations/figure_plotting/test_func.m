...
function RMSE = testFunction_for_students_MTb(teamName)
    ...
    % To see rmse_over_time plot, set to true! The figure in the report assumes perfect cls
    % so uncomment line 42 predictedDir = trueDir; to reproduce figure
    rmse_over_time = false; 
    ...
    % These plots require stat and ml toolbox
    ...
    if rmse_over_time == true
        plot_rmse_over_time(sqErrorByTime, countsByTime, time_from_onset)
    end
...
function plot_rmse_over_time(sqErrorByTime, countsByTime, time_from_onset)
